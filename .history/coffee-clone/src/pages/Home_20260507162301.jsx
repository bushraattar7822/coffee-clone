...
function Marquee() {
  ...
  return (
    <div className="overflow-hidden bg-white py-6 border-y border-gray-100">
      <motion.div
      ...
      >
        {repeated.map((item, i) => (
          <div key={i} className="flex items-center gap-4 shrink-0">
            ...
            <span className="text-2xl font-bold text-joe-dark tracking-tight">{item.label}</span>
          </div>
        ))}
      </motion.div>
    </div>
...
<section
  ...
  style={{
  backgroundColor: "#c7b4d6",
  backgroundImage: `
    radial-gradient(circle at 20% 70%, rgba(229,170,145,0.75) 0%, transparent 32%),
    radial-gradient(circle at 70% 35%, rgba(165,140,205,0.9) 0%, transparent 40%),
    radial-gradient(circle at 80% 60%, rgba(240,185,170,0.45) 0%, transparent 28%),
    linear-gradient(
      135deg,
      #c8b7dc 0%,
      #b39ccc 25%,
...
>
  ...
  <div
    ...
    style={{
      backgroundImage:
        "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noise)' opacity='0.35'/%3E%3C/svg%3E\")",
    }}
  />
  ...
  <div className="relative z-10 w-full max-w-[980px] px-6 -mt-8">
    ...
    {/* HERO TEXT */}
    <div className="max-w-[760px] mx-auto text-left text-white relative">

      {/* top line */}
      <p className="text-[26px] lg:text-[38px] font-black tracking-tight leading-none mb-1">
      ...
      {/* BIG TITLE */}
      <h1
        className="font-black tracking-[-0.05em] leading-[0.88] mb-3"
        style={{
      ...
      {/* paragraph */}
      <p className="text-[15px] lg:text-[20px] leading-[1.25] font-black max-w-[820px]">
        who grow, harvest, invest, and transform. moving into feminist futures within coffee. profound positive change, courage curiosity, and collective power{" "}
        <span className="line-through opacity-60">
          an idea to agree with
        </span>
      </p>

      {/* huge movement text */}
      <div
        className="font-black tracking-[-0.06em] leading-[0.85] mt-1"
        style={{
          fontSize: "clamp(70px, 10vw, 145px)",
        }}
      >
        a movement to join
      </div>
      ...
      <div className="mt-3 text-center">
        <Link
          to="/shop"
          className="inline-block border-b border-white text-white text-[12px] font-black tracking-[0.2em] uppercase pb-1 hover:opacity-70 transition-opacity"
        >
          Learn More + Shop Café Feminista
        </Link>
      </div>
    </div>
  </div>
