...
function FakeMap() {
  return (
    <div className="relative overflow-hidden bg-[#dff5ff]">
      <div
        className="absolute inset-0 opacity-70"
        style={{
          backgroundImage:
            "linear-gradient(28deg, transparent 47%, #89cff2 48%, #89cff2 50%, transparent 51%), linear-gradient(115deg, transparent 47%, #89cff2 48%, #89cff2 50%, transparent 51%), linear-gradient(#bfeaff 1px, transparent 1px), linear-gradient(90deg, #bfeaff 1px, transparent 1px)",
          backgroundSize: "90px 90px, 120px 120px, 34px 34px, 34px 34px",
        }}
      />

      {[["4", "35%", "18%"], ["2", "32%", "36%"], ["10", "28%", "58%"], ["4", "26%", "78%"]].map(
        ([num, left, top]) => (
          <div
            key={`${num}-${top}`}
            className="absolute flex h-[54px] w-[54px] items-center justify-center rounded-full border-2 border-[#00749a] bg-[#118acb] text-[18px] font-black text-white shadow"
            style={{ left, top }}
          >
            {num}
          </div>
      ...
      <div className="absolute right-4 top-4 overflow-hidden rounded bg-white shadow">
        <button className="flex h-9 w-9 items-center justify-center border-b">
          <Plus size={20} />
        </button>
