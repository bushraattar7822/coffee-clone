...
const img = {
  ...
  hero:
    "https://joecoffeecompany.com/cdn/shop/files/Untitled-2_5a6649fa-7980-4491-ba92-e591c4af2c46.jpg?v=1774298563&width=1200",
  coffee1:
    "https://joecoffeecompany.com/cdn/shop/files/12ozcopy_7c854987-812b-46f2-b726-bea9bef1af5e.jpg?v=1773075521&width=533",
  coffee2:
    "https://joecoffeecompany.com/cdn/shop/files/image0copy.jpg?v=1773928249&width=533",
};
...
function MegaContent({ label }) {
  ...
  const data = {
    CLASSES: {
      ...
      cards: [
        { image: img.twa, button: "UPCOMING CLASSES" },
        { image: img.pickup, button: "PROFESSIONAL TRAINING" },
      ],
    },
    CATERING: {
      title: "CATERING",
      ...
      cards: [
        { image: img.coffee1, button: "CATER WITH JOE" },
        { image: img.hero, button: "OUR SERVICES" },
      ],
    },
    WHOLESALE: {
      title: "WHOLESALE",
      ...
      cards: [
        { image: img.bundle, button: "OUR COFFEE PROGRAM" },
        { image: img.pickup, button: "OUR TECH SERVICES" },
      ],
    },
  }[label];
    ...
    <div className="px-[65px] py-10 min-h-[498px] grid grid-cols-[277px_287px_287px_367px] gap-0">
      ...
      <Link to="/shop" className="block">
        <div className="relative h-[365px] w-[367px] overflow-hidden bg-[#83d5df]">
          ...
          <img src={img.bundle} alt="" className="h-full w-full object-cover" />
        </div>

        <p className="mt-5 text-[16px] font-extrabold">Build Your Own Coffee Bundle</p>
...
function LocationsMega() {
  return (
    <div className="px-[65px] py-10 min-h-[430px] grid grid-cols-[260px_270px_1fr_1fr] gap-10">
      ...
      <ImageTitle image={img.twa} title="Visit Joe at the TWA Hotel" />
      <ImageTitle image={img.pickup} title="Order Local Pick Up" />
    </div>
  );
}
...
function DiscoverMega() {
  return (
    <div className="px-[65px] py-10 min-h-[390px] grid grid-cols-[300px_1fr_1fr] gap-[65px]">
      ...
      <ImageButton image={img.hero} button="HUMANITIES STATEMENT" />
      <ImageButton image={img.coffee2} button="WE'RE HIRING" />
    </div>
  );
}
...
function ImageButton({ image, button }) {
  return (
    <Link to="/shop" className="relative block h-[309px] overflow-hidden">
      ...
      <span className="absolute bottom-[30px] left-1/2 -translate-x-1/2 h-[46px] min-w-[230px] rounded-full bg-[#118acb] text-white text-[15px] font-extrabold flex items-center justify-center px-8">
        {button}
      </span>
    </Link>
  );
...
function ImageTitle({ image, title }) {
  return (
    <Link to="/shop" className="block">
      ...
      <p className="mt-4 text-[16px] font-extrabold">{title}</p>
    </Link>
  );
}
...
function Underlined({ children }) {
  return (
    <h3 className="mb-[7px] w-fit border-b border-[#303437] text-[18px] font-extrabold uppercase leading-tight text-[#303437]">
      {children}
    </h3>
  );
}
    ...
    <div className="space-y-[5px]">
      {items.map((item) => (
        <Link
          key={item}
          to="/shop"
          className="block text-[16px] font-extrabold leading-[1.28] text-[#303437]"
        >
          {item}
        </Link>
      ))}
